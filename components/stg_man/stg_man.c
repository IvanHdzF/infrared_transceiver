...
#define STG_MAN_MAX_CHAR_SIZE    300
...
static esp_err_t s_example_read_file(char *path)
{
    ...
    char line[STG_MAN_MAX_CHAR_SIZE];
...
esp_err_t stg_man_init()
{
    ...
    char data[STG_MAN_MAX_CHAR_SIZE];
    snprintf(data, STG_MAN_MAX_CHAR_SIZE, "%s %s\n", "hello world, from ESP-IDF", esp_get_idf_version());
    ...
    //Create file and write
    memset(data, 0, STG_MAN_MAX_CHAR_SIZE);
