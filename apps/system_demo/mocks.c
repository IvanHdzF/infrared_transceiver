...
typedef struct {
    os_link_state_t ble_up;
} mock_ble_t;
typedef struct {
    os_link_state_t wifi_up;
} mock_wifi_t;
typedef struct {
    os_power_mode_t pwr;
} mock_power_t;
...
{
    ESP_LOGI(TAG, "mock_ir_init");
    return OS_OK;
}
os_err_t mock_sched_init(void)
{
    ESP_LOGI(TAG, "mock_sched_init");
    return OS_OK;
}
os_err_t mock_storage_init(void)
{
    ESP_LOGI(TAG, "mock_storage_init");
    return OS_OK;
}
os_err_t mock_clock_init(void)
{
    ESP_LOGI(TAG, "mock_clock_init");
    return OS_OK;
}
os_err_t mock_cmd_init(void)
{
    ESP_LOGI(TAG, "mock_cmd_init");
    return OS_OK;
}
os_err_t mock_orch_init(void)
{
    ESP_LOGI(TAG, "mock_orch_init");
    return OS_OK;
}
os_err_t mock_errmgr_init(void)
{
    ESP_LOGI(TAG, "mock_errmgr_init");
    return OS_OK;
}
os_err_t mock_event_bus_init(void)
{
    ESP_LOGI(TAG, "mock_event_bus_init");
    return OS_OK;
}
