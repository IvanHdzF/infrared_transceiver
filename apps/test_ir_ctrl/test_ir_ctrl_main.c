...
#define IR_CTRL_DUT_DUMMY_TX_GPIO  1
#endif

#ifndef IR_CTRL_DUT_DUMMY_RX_GPIO
#define IR_CTRL_DUT_DUMMY_RX_GPIO  1
#endif
...
static void wait_until_busy_or_fail(int timeout_ms)
{
    ...
    TEST_ASSERT_TRUE_MESSAGE(ir_ctrl_is_busy(), "did not enter busy in time");
}

static void wait_until_not_busy_or_fail(int timeout_ms)
{
    ESP_LOGI(TAG, "Waiting up to %d ms for ir_ctrl to exit busy...", timeout_ms);
    ...
    TEST_ASSERT_FALSE_MESSAGE(ir_ctrl_is_busy(), "did not exit busy in time");
}
...
static void rmt_frame_invert_levels(const rmt_symbol_word_t *in, uint16_t in_n, rmt_symbol_word_t *out)
{
...
static void sniffer_postprocess_like_learn(rmt_frame_obj_t *frame, bool invert_levels, bool normalize)
{
    TEST_ASSERT_NOT_NULL(frame);
    ...
    if (normalize) {
        uint16_t out_n = 0;
        ir_post_nec_canon(cur, n, tmp_norm, &out_n, NULL);
        TEST_ASSERT_TRUE_MESSAGE(out_n > 0 && out_n <= IR_CTRL_MAX_FRAME_SIZE, "postprocess produced invalid length");
        memcpy(frame->rmt_frame_data, tmp_norm, out_n * sizeof(rmt_symbol_word_t));
        frame->num_syms = out_n;
    } else if (cur != frame->rmt_frame_data) {
        memcpy(frame->rmt_frame_data, cur, n * sizeof(rmt_symbol_word_t));
        frame->num_syms = n;
    }
}

TEST_CASE("ir_ctrl: learn slot1 while external stim transmits golden (loopback)", "[ir_ctrl]")
{
    storage_reset();

    ESP_LOGI(TAG, "Loopback: TX(GPIO%d) -> RX(GPIO%d)", IR_CTRL_TX_GPIO, IR_CTRL_RX_GPIO);

    const uint16_t slot1 = 1;

    ir_ctrl_cfg_t dut_cfg = {
        .resolution = IR_CTRL_RES_HZ,
        .tx_gpio_num = IR_CTRL_DUT_DUMMY_TX_GPIO,
    ...
    ir_stim_t stim;
    ir_stim_init(&stim, IR_CTRL_TX_GPIO, IR_CTRL_RES_HZ, true, 38000, 0.33f, false);
    ir_stim_send_frame_blocking(&stim, &nec_golden_frame, true, 1000);
    ir_stim_deinit(&stim);
    ...
    ESP_LOGI(TAG, "PASS: slot1 captured externally-stimulated waveform");
    TEST_ASSERT_EQUAL(ESP_OK, ir_ctrl_deinit());
}

TEST_CASE("ir_ctrl: send slot0 and sniffer captures on RX (loopback)", "[ir_ctrl]")
{
    storage_reset();
    ...
    ir_ctrl_cfg_t dut_cfg = {
        ...
        .rx_gpio_num = IR_CTRL_DUT_DUMMY_RX_GPIO,
        .decode_margin = 0,
        .store_frame_func = test_store,
        .load_frame_func = test_load,
    };
    ...
    ir_sniffer_t snf;
    ir_sniffer_init(&snf, IR_CTRL_RX_GPIO, IR_CTRL_RES_HZ, false, sniff_buf, sizeof(sniff_buf));
    ...
    TEST_ASSERT_TRUE_MESSAGE(ed.num_symbols <= IR_CTRL_MAX_FRAME_SIZE, "sniffer overflow");

    rmt_frame_obj_t sniffed = {0};
    sniffed.num_syms = (uint16_t)ed.num_symbols;
    memcpy(sniffed.rmt_frame_data, ed.received_symbols, ed.num_symbols * sizeof(rmt_symbol_word_t));

    sniffer_postprocess_like_learn(&sniffed, true, true);
    assert_frame_matches_golden_with_trailer_tolerance(&sniffed, &nec_golden_frame);

    ir_sniffer_deinit(&snf);

    ESP_LOGI(TAG, "PASS: sniffer captured DUT TX waveform");
    TEST_ASSERT_EQUAL(ESP_OK, ir_ctrl_deinit());
}

// Extra checks (state-machine focused):
// - learn_cancel should clear busy and allow subsequent init/deinit cycles.
// - get_slot_info should reject while busy (avoids racing storage callbacks vs capture/send).

TEST_CASE("ir_ctrl: learn_cancel clears busy", "[ir_ctrl]")
{
    storage_reset();
    ...
    TEST_ASSERT_EQUAL(ESP_OK, ir_ctrl_learn_start(1, &learn_opts));
    wait_until_busy_or_fail(500);

    TEST_ASSERT_EQUAL(ESP_OK, ir_ctrl_learn_cancel());
    wait_until_not_busy_or_fail(500);

    TEST_ASSERT_EQUAL(ESP_OK, ir_ctrl_deinit());
}

TEST_CASE("ir_ctrl: get_slot_info rejects while busy", "[ir_ctrl]")
{
    storage_reset();
    ...
    rmt_frame_obj_t tmp = {0};
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE, ir_ctrl_get_slot_info(1, &tmp));
